...
export function Admin() {
  ...
  const handleSizeChange = (index: number, field: 'size' | 'stock', value: string | number) => {
    const newSizes = [...sizes]; newSizes[index] = { ...newSizes[index], [field]: value }; setSizes(newSizes);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); setIsLoading(true); setSuccess(false);
    try {
      const data = new FormData();
      data.append('name', formData.name); data.append('description', formData.description);
      data.append('price', formData.price); data.append('mrp', formData.mrp);
      data.append('category', formData.category); data.append('badge', formData.badge);
      ...
      images.forEach((img, i) => { if (img) data.append(`image${i + 1}`, img); });
      await api.post('/admin/products', data, { headers: { 'Content-Type': 'multipart/form-data' } });
      setSuccess(true);
      setFormData({ name: '', description: '', price: '', mrp: '', category: 'Essential', badge: '', isFeatured: false, tags: '' });
      setImages([null, null, null, null, null]);
    } catch (err: any) { alert(err.response?.data?.error || 'Failed to create product'); }
  ...
  return (
    <div className="min-h-screen bg-[#121212] text-white pt-[140px] pb-20 px-6 max-w-[1600px] mx-auto overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 admin-anim">
        ...
        {success && (
          <div className="glass-dark border border-[#c8ff00]/30 px-6 py-4 mt-8 md:mt-0 flex items-center gap-4">
      ...
      {/* Tab Navigation */}
      <div className="flex gap-2 mb-12 admin-anim">
        <button onClick={() => setActiveTab('orders')}
          className={`flex items-center gap-3 px-8 py-4 font-body text-[12px] uppercase tracking-[0.2em] font-black transition-all border ${activeTab === 'orders' ? 'bg-[#c8ff00] text-black border-[#c8ff00]' : 'glass-dark border-white/10 text-white/60 hover:border-white/30'}`}>
        ...
        {activeTab === 'orders' ? (
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20">
                ...
                <div className="grid grid-cols-1 gap-10">
                  <div className="space-y-8">
                    <div className="group">
                      ...
                      <input required name="name" value={formData.name} onChange={handleInputChange} placeholder="Enter product title..." className="w-full pb-3 bg-transparent border-b border-white/20 font-display text-[24px] uppercase text-white outline-none focus:border-[#d1ff00] transition-all placeholder:text-white/10" />
                    </div>
                    <div className="grid grid-cols-2 gap-8">
                      <div className="group">
                        ...
                        <select name="category" value={formData.category} onChange={handleInputChange} className="w-full pb-3 bg-transparent border-b border-white/20 font-body text-[14px] text-white outline-none focus:border-[#c8ff00] transition-all appearance-none cursor-pointer">
                    ...
                    <div className="group">
                      ...
                      <textarea required name="description" value={formData.description} onChange={handleInputChange} rows={4} className="w-full p-4 glass-dark border border-white/10 font-body text-[14px] text-white/80 outline-none focus:border-[#c8ff00] resize-none transition-all placeholder:text-white/20" placeholder="Describe the fabric, fit, and soul of this piece..." />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-8">
                    <div className="group">
                  ...
                  {sizes.map((s, idx) => (
                    <div key={idx} className="flex gap-8 items-center group">
                      <div className="flex-1">
                        <input value={s.size} onChange={(e) => handleSizeChange(idx, 'size', e.target.value)} className="w-full pb-2 bg-transparent border-b border-white/10 font-body text-[14px] font-bold uppercase text-white outline-none focus:border-[#c8ff00] transition-all" placeholder="SIZE_CODE" />
                      </div>
                      <div className="w-32">
                        <input type="number" value={s.stock} onChange={(e) => handleSizeChange(idx, 'stock', parseInt(e.target.value))} className="w-full pb-2 bg-transparent border-b border-white/10 font-body text-[14px] text-white/60 outline-none focus:border-[#c8ff00] transition-all" placeholder="STOCK_UNIT" />
                      </div>
                      <button type="button" onClick={() => setSizes(sizes.filter((_, i) => i !== idx))} className="p-3 text-white/20 hover:text-red-500 transition-colors"><Trash2 size={16} /></button>
                    </div>
                  ))}
                  <button type="button" onClick={() => setSizes([...sizes, { size: '', stock: 0 }])} className="flex items-center gap-3 font-body text-[11px] uppercase tracking-[0.2em] font-black text-[#c8ff00] hover:brightness-125 transition-all mt-8">
            ...
            <div className="lg:col-span-5 space-y-12">
                      ...
                      {img ? (
                        <div className="absolute inset-0">
                          ...
                          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all">
                            <button type="button" onClick={() => handleImageChange(idx, { target: { files: null } } as any)} className="w-12 h-12 glass-dark rounded-full flex items-center justify-center text-[#c8ff00] hover:bg-[#c8ff00] hover:text-black transition-all"><Trash2 size={20} /></button>
                          </div>
                        </div>
                      ) : (
              ...
              <div className="space-y-6">
                <button type="submit" disabled={isLoading} className="w-full py-7 bg-[#c8ff00] text-black font-body font-black text-[14px] uppercase tracking-[0.5em] hover:brightness-110 transition-all shadow-[0_0_40px_rgba(200,255,0,0.1)] active:scale-[0.99] disabled:opacity-50">
